...
def run_action(user_prompt, history, game_state):
    ...
    messages_as_text_parts = [
        types.Part.from_text(f"{message}") for message in messages
    ]
    ...
    game_master_response = " ".join(
        [f"{value}" for value in response_dict.values()]
    )
...
def main_loop(user_prompt, history):
    ...
    if not policy.is_safe(
        game_master_response, game_state["age_appropriate_rating"]
    ):
        return f"Response is not appropriate for this game's rating of {game_state["age_appropriate_rating"]}."

    item_updates = inventory.detect_inventory_changes(
        game_state, game_master_response
    )
    update_msg = inventory.update_inventory(
        game_state["inventory"], item_updates
    )
...
if __name__ == "__main__":
    ...
    game_state = game.get_starting_game_state(
        game_configuration.load_configuration_file(
            os.getenv("WORLD_MODEL_PATH")
        ),
        game_configuration.load_configuration_file(
            os.getenv("GAME_SCENARIO_PATH")
        ),
    )
